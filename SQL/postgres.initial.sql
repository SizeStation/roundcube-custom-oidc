...
CREATE TABLE sizestation_mailbox_assignments (
    ...
    issuer varchar(255) NOT NULL,
    external_user_id varchar(255) NOT NULL,
    principal_id bigint REFERENCES sizestation_oidc_principals(id),
    ...
    is_anchor smallint NOT NULL DEFAULT 0 CHECK(is_anchor IN (0, 1)),
    is_preferred smallint NOT NULL DEFAULT 0 CHECK(is_preferred IN (0, 1)),
...
CREATE TABLE sizestation_oidc_audit_log (
    id bigserial PRIMARY KEY,
    principal_id bigint REFERENCES sizestation_oidc_principals(id),
    assignment_id varchar(36) REFERENCES sizestation_mailbox_assignments(id),
...
(
    ...
    iid integer NOT NULL
        REFERENCES identities(identity_id) ON DELETE CASCADE ON UPDATE CASCADE UNIQUE,
    parent_id integer DEFAULT NULL,
    username varchar(64),
    password varchar(255),
    imap_host varchar(64),
    imap_port integer CHECK(imap_port > 0 AND imap_port <= 65535),
    ...
    sieve_port integer CHECK(sieve_port > 0 AND sieve_port <= 65535),
    ...
    managed_externally smallint NOT NULL DEFAULT 0
        CHECK(managed_externally IN (0, 1)),
    managed_assignment_id varchar(36) UNIQUE,
